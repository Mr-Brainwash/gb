	...
	<div class="container">
		<div class="row">
			<div class="offset-md-2 mt-3">
				...
				if(isset($_GET['id']))
				{
					$sql	=	$dbh->query('UPDATE `images` SET `views_count` = `views_count` + 1 WHERE `id` = '.$_GET['id'].'');
					$result	=	$dbh->query('SELECT * FROM images WHERE id = '.$_GET['id'].'');
					foreach ($result as $res) {
						$image  = IMG_DIR . $res['id'] . $res['imgname'] .'.'. $res['extension'];
						echo'<img src="'.$image.'" alt="'.$res['imgname'].'"  width="600" height="500" class="card-img-top">';
						echo '<div class="alert alert-light" role="alert">Количество просмотров: '.$res['views_count'].'</div>';
					}
				}	
				?>
				</div>
